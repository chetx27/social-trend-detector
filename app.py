...
def trends():
    ...
    try:
        ...
        viral_trends = cursor.execute(
            'SELECT id, platform, text, engagement_score, anomaly_score FROM trends ORDER BY anomaly_score DESC LIMIT 10'
        ).fetchall()
...
def posts():
    ...
    try:
        ...
        posts_list = cursor.execute(
            'SELECT id, platform, text, engagement_score FROM posts_processed ORDER BY engagement_score DESC LIMIT 20'
        ).fetchall()
