...
class TrendDetector:
    ...
    def detect_anomalies(self):
        ...
        posts = self.cursor.execute(
            'SELECT id, platform, text, cleaned_text, engagement_score FROM posts_processed WHERE engagement_score IS NOT NULL'
        ).fetchall()
        ...
        for i, post in enumerate(posts):
            if anomalies[i] == -1:
                post_id, platform, text, cleaned_text, engagement = post
                viral_posts.append((
                    platform, text, cleaned_text, engagement, 1, float(scores[i][0])
                ))
        ...
        if viral_posts:
            ...
            self.cursor.executemany(
                '''INSERT INTO trends 
                (platform, text, cleaned_text, engagement_score, is_viral, anomaly_score)
                VALUES (?, ?, ?, ?, ?, ?)''',
                viral_posts
            )
            self.conn.commit()
